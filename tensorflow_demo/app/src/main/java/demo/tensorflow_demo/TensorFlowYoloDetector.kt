...
import demo.tensorflow_demo.env.SplitTimer
    ...
    override fun recognizeImage(bitmap: Bitmap): List<Classifier.Recognition> {
        ...
        for (y in 0..gridHeight - 1) {
            for (x in 0..gridWidth - 1) {
                for (b in 0..NUM_BOXES_PER_BLOCK - 1) {
                    ...
                    if (confidenceInClass > 0.01) {
                        ...
                        pq.add(Classifier.Recognition("" + offset, LABELS[detectedClass], confidenceInClass, rect))
                    }
                }
            }
        }
