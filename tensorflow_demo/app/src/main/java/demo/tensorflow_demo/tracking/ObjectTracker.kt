...
import demo.tensorflow_demo.env.Size
...
class ObjectTracker protected constructor(
    ...
    private fun drawHistoryPoint(canvas: Canvas, startX: Float, startY: Float) {
        ...
        synchronized(debugHistory) {
            ...
            var lastX = startX
            var lastY = startY
            for (keypointNum in 0..numPoints - 1) {
    ...
    private fun drawKeypointsDebug(canvas: Canvas) {
        ...
        for (keypoint in lastKeypoints!!.pointDeltas) {
            if (keypoint.wasFound) {
                ...
                val color = 0xFF000000.toInt() or (r shl 16) or b
                p.color = color
    ...
    @Synchronized private fun getAccumulatedDelta(timestamp: Long, positionX: Float,
    ...
    @Synchronized private fun getCurrentPosition(timestamp: Long, oldPosition: RectF): RectF {
    ...
    inner class TrackedObject internal constructor(position: RectF, timestamp: Long, data: ByteArray) {
        ...
        @get:Synchronized internal var lastExternalPositionTime: Long = 0
            private set
        ...
        @Synchronized internal fun setPreviousPosition(position: RectF, timestamp: Long) {
            checkValidObject()
            synchronized(this@ObjectTracker) {
        ...
        @Synchronized fun updateTrackedPosition() {
            checkValidObject()
